...
?>
  ...
  <form class="form">
    ...
    <div>
    <input type="checkbox" id="pwd" onclick="toggleCheckbox('pwd')">
    <label for="pwd">PWD</label> |
    <input type="checkbox" id="senior" onclick="toggleCheckbox('senior')">
...
function toggleCheckbox(id) {
    ...
    function applyDiscount() {
        const totalElement = document.getElementById('total_orders');
        const originalText = totalElement.getAttribute('data-original-value');
        let totalText;

        if (originalText !== null) {
            totalText = originalText;
        } else {
            totalText = totalElement.textContent.replace('Total: ', '').replace('₱', '');
            totalElement.setAttribute('data-original-value', totalText);
        }

        let totalValue = parseFloat(totalText);

        if (isNaN(totalValue)) {
            totalValue = 0; // Set default value if parsing fails
        }

        const pwdCheckbox = document.getElementById('pwd');
        const seniorCheckbox = document.getElementById('senior');

        if (pwdCheckbox.checked || seniorCheckbox.checked) {
            totalValue = totalValue * 0.80; // Apply 20% discount
        } else {
            totalValue = parseFloat(originalText); // Reset to original value if no checkbox is checked
        }
    ...
    var payment_receive = $('#customers_payment').val();
    ...
    if (pwdCheckbox.checked || seniorCheckbox.checked) {
        ...
        var totalAmountWithDiscount = totalOrders / 0.80;
        // Calculate the actual discount
        discount = totalAmountWithDiscount - totalOrders;
    }

    var receiptContent = '<div style="width: 58mm; margin-left:-9px !important; margin-top:-18px !important; text-align: left; font-size: 15; overflow:hidden; font-family:arial !important;">';
    receiptContent += '<h3 style="text-align: center; font-size:18px;">Order #' + currentOrderNumber + '</h3>';
    receiptContent += '<p style="font-size:15px;">Order Date: ' + new Date().toLocaleString() + '</p><br>';

    receiptContent += '<table style="width: 100%; margin-top: 10px; border-collapse: collapse; border-bottom:0.5px solid grey; font-size:10px;">';
    receiptContent += '<thead>';
    receiptContent += '<tr>';
    receiptContent += '<th style="border-bottom: 0.5px solid #8E8E8E; text-align:center; border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">Qty</th>';
    receiptContent += '<th style="border-bottom: 0.5px solid #8E8E8E; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">Order/s</th>';
    receiptContent += '<th style="border-bottom: 0.5px solid #8E8E8E; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">Price</th>';
    receiptContent += '<th style="border-bottom: 0.5px solid #8E8E8E; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">Total</th>';
    receiptContent += '</tr>';
    receiptContent += '</thead>';
    ...
    $('#orders_table tr').each(function() {
        ...
        if (!isNaN(quantity) && !isNaN(totalAmount) && !isNaN(price)) {
            receiptContent += '<tr style="font-size:10px;">';
            receiptContent += '<td style="border-right: 0.5px solid transparent; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">' + quantity + '</td>';
            receiptContent += '<td style="border-right: 0.5px solid transparent; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">' + foodName + '</td>';
            receiptContent += '<td style="border-right: 0.5px solid transparent; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">' + price + '</td>';
            receiptContent += '<td style="border-right: 0.5px solid transparent; text-align:center;border-left: 0.5px solid transparent; padding: 4px; font-size:15; font-weight:400;">' + totalAmount + '</td>';
            receiptContent += '</tr>';
        }
    });
    ...
    // Display different sections based on discount application
    if (discountApplied) {
        // Display real amount before discount
        receiptContent += '<p style="text-align:right;">Real Amount:<b> ' + totalOrders.toFixed(2) + '</b></p>';
        // Display discount if applied
        receiptContent += '<p style="text-align:right;">Discount:<b> ' + discount.toFixed(2) + '</b></p>';
        // Display total amount after discount
        receiptContent += '<p style="text-align:right;">Total Amount After Discount:<b> ' + totalAmountWithDiscount.toFixed(2) + '</b></p>';
    } else {
        // Display total amount without discount
        receiptContent += '<p style="text-align:left; font-size:15px;">Total Amount:<b> ' + totalOrders.toFixed(2) + '</b></p>';
    }

    // Display received amount and change
    receiptContent += '<p style="text-align:left; font-size:15px;border-bottom:0.5px solid grey;">Received Amount:<b> ' + payment_receive + '</b></p>';
    receiptContent += '<p style="text-align:left; font-size:15px;"><b>' + customer_change + '</b></p>';

    receiptContent += '<h3 style="text-align:left; font-size:15px;">#' + badgeNumber + '</h3><br>';
    receiptContent += '</div>';
